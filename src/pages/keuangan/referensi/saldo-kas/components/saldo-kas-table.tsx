...
import SaldoKasEntity from "@/pages/keuangan/referensi/saldo-kas/data/saldo-kas.entity.ts";



export default function SaldoKasTable({handleGroupModal, params, setSelectedData, setDetail}: {
    params: IUseParams,
    handleGroupModal: (key: string, value: boolean) => void,
    setSelectedData: Dispatch<SaldoKasEntity>
    setDetail: Dispatch<{ key: string, value: string }[]>
}) {


    const {data, isLoading} = useGetList<SaldoKasEntity[]>({
        endpoint: "/saldo-kas",
        name: "saldo-kas",
        params
    })

    function handleEditClick(data: SaldoKasEntity) {
        setSelectedData(data)
        handleGroupModal("modal", true)
    }

    function handleDeleteClick(data: SaldoKasEntity) {
        deleteAlert({
            data,
            handleSubmit: () => {
    ...
    function handleDetailClick(data: SaldoKasEntity) {
    ...
    const columns: TableProps<SaldoKasEntity>['columns'] = [
        {
            title: 'Nama Sekolah',
            dataIndex: 'sekolah',
            // width: '25%',
